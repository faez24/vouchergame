...
<div class="relative z-10 flex-1 max-w-[780px] mx-auto w-full px-4 pt-28 pb-36">
  ...
  {{-- Filter bar --}}
  <div id="filter-bar" class="hidden mb-4 flex gap-2 overflow-x-auto pb-2 scrollbar-hide" style="-ms-overflow-style:none;scrollbar-width:none;">
    <!-- Generated by JS -->
  </div>
...
{{-- Edit User ID Modal --}}
<div id="edit-modal" class="hidden fixed inset-0 z-[130] items-center justify-center p-4">
  <div class="absolute inset-0 bg-black/70 backdrop-blur-sm" onclick="closeEditModal()"></div>
  <div class="relative w-full max-w-sm bg-[#1e2433] border border-white/10 rounded-3xl shadow-[0_30px_80px_rgba(0,0,0,.8)] p-6" style="animation:fadeUp .25s ease">
    <h3 class="text-white font-black text-lg mb-5">Edit Data Akun</h3>
...
function setFilter(g) { window.activeFilter = g; renderCart(); }

function openEditModal(id) {
    const item = getCart().find(i => i.id === id);
    if(item) {
        document.getElementById('edit-item-id').value = id;
        document.getElementById('edit-uid').value = item.userId;
        document.getElementById('edit-zone').value = item.zoneId;
...
function saveEdit() {
    ...
    if(!uid || !zone) { toast('⚠️', 'Data tidak boleh kosong'); return; }
    
    let cart = getCart();
    let idx = cart.findIndex(i => i.id === id);
    if(idx !== -1) {
        cart[idx].userId = uid;
        cart[idx].zoneId = zone;
        saveCart(cart);
        closeEditModal();
        renderCart();
...
function renderCart(){
  ...
  // Render Filters
  const filtersContainer = document.getElementById('filter-bar');
  ...
  // Filter the items to render
  const filteredCart = window.activeFilter === 'All' ? cart : cart.filter(i => i.game === window.activeFilter);

  filteredCart.forEach((item,i)=>{
    const div=document.createElement('div');
    div.id='ci-'+item.id;
    div.className='card-item cursor-pointer fade-up bg-[#252d40]/80 backdrop-blur-md border border-white/8 rounded-2xl p-4 flex items-center gap-3';
    div.style.animationDelay=(i*.05)+'s';div.style.opacity='0';
    div.onclick = (e) => {
      // Prevent double toggle if clicking directly on checkbox, and ignore if clicking the remove button
      if (e.target.tagName === 'INPUT' || e.target.closest('button')) return;
      const cb = div.querySelector('.item-chk');
      if (cb) {
    ...
    div.innerHTML=`
      ...
      <button onclick="removeItem('${item.id}'); event.stopPropagation();" class="w-8 h-8 rounded-lg bg-white/5 hover:bg-red-500/20 hover:text-red-400 text-gray-500 flex items-center justify-center transition-all flex-shrink-0">
...
function confirmPayment(){
  ...
  const remaining=cart.filter(i=>!ids.includes(i.id));
  saveCart(remaining);
